...
results_df = spark.read.format("delta").load(f"{processed_folder_path}/results").withColumnRenamed("time", "race_time").withColumnRenamed("race_id", "result_race_id").withColumnRenamed("file_date", "result_file_date")

results_df = results_df.filter(results_df.result_file_date == f"{v_file_date}")
